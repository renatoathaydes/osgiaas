...
class ArgsSpecSpecification extends Specification {
    ...
    def "Completer can be used to auto-complete a command with enumerated argument values"() {
        given: 'An ArgsSpec is built with a few enumerated values options'
        def spec = ArgsSpec.builder()
                .accepts( '-f' ).mandatory().withEnumeratedArgs( [ hi: { [ 'yes', 'no' ] } as Supplier ] ).end()
                .accepts( '-n' ).withOptionalEnumeratedArgs( [ ho: { [ 'high', 'low' ] } as Supplier ] ).end()
                .build()
        ...
        'hi -n -'            | 6             | [ '-f', ]
        'hi -n hi'           | 6             | [ 'high' ]
        'hi -f yes -n '      | 13            | [ 'high', 'low' ]
        'hi -f yes -n h'     | 13            | [ 'high' ]
        'hi -n high -f y'    | 14            | [ 'yes' ]
        'hi -f x'            | -1            | [ ]
        'hi -n x'            | -1            | [ ]
        'hi -f x '           | -1            | [ ]
        'hi -f x -'          | -1            | [ ]
